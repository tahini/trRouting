...
namespace TrRouting
{
  ...
  public:
    ...
    // TODO This function supporst both allNodes and simple calculation, which
    // are 2 very different return values. They should be split so it can return
    // a single type of result (that alternatives could use directly), but still
    ...
    std::unique_ptr<RoutingResult> alternativesRouting(RouteParameters &parameters);
