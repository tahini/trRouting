...
TEST_F(ResultToV2FixtureTest, TestNoRoutingFoundResult2)
{
    TrRouting::ResultToV2Response converter = TrRouting::ResultToV2Response();
    nlohmann::json jsonResponse = converter.noRoutingFoundResponse(*testParameters.get());
    ...
    ASSERT_EQ(testParameters.get()->getTimeOfTrip(), jsonResponse["requestTime"]);
    ASSERT_EQ(0, jsonResponse["timeType"]);
}

TEST_F(ResultToV2FixtureTest, TestSingleCalculationResult2)
{
    ...
    TrRouting::ResultToV2Response converter = TrRouting::ResultToV2Response();
    nlohmann::json jsonResponse = converter.resultToJsonString(result, *testParameters.get());

    assertResultConversion(jsonResponse, result, *testParameters.get());
}

TEST_F(ResultToV2FixtureTest, TestAlternativesResult2)
{
...
void ResultToV2FixtureTest::assertResultConversion(nlohmann::json jsonResponse, TrRouting::SingleCalculationResult &result, TrRouting::RouteParameters &params) {
    ...
    ASSERT_EQ(destination->latitude, jsonResponse["destination"][0]);
    ASSERT_EQ(destination->longitude, jsonResponse["destination"][1]);
    ASSERT_EQ(params.getTimeOfTrip(), jsonResponse["requestTime"]);
    ASSERT_EQ(0, jsonResponse["timeType"]);
